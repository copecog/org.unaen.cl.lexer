...
(defgeneric push-state-next (FA &key &allow-other-keys)
  (:documentation "Push the next implied by state-names under FA."))
...
(defmethod push-state ((next (eql 'next)) (FA-instance FA) &key (start-p nil) (final-p nil)
			       &allow-other-keys)
...
(defmethod push-state ((state integer) (FA-instance FA) &key (start-p nil) (final-p nil)
		       &allow-other-keys)
  (with-slots (Q) FA-instance
    (if (<= state (fill-pointer Q))
	(values FA-instance state)
	(values FA-instance nil))))
...
(defmethod push-state :before ((state-name string) (q₀-instance q₀) &key (start-p nil)
			       &allow-other-keys)
  (with-slots (q₀) q₀-instance
    (when start-p
      (setf q₀ state-name)))) ; If start state then set as start state.

(defmethod push-state :before ((state-name string) (F-instance F) &key (final-p nil)
			       &allow-other-keys)
...
(defmethod push-state-next ((FA-instance FA) &key (start-p nil) (final-p nil) &allow-other-keys)
  (push-state (make-state-name (dsn FA-instance))
		               FA-instance
		               :start-p start-p
		               :final-p final-p))
...
(defmethod get-transit ((state integer)
  ...
  (with-slots (Δ) Δ-instance
    (let* ((Δ.state (aref Δ state))
	   (Δ.state.transit-char (gethash transit-char Δ.state)))
      Δ.state.transit-char)))
...
(defmethod push-fragment ((fragment-type (eql 'regex-literal))
			  ...
			  &key
			    ...
			    (end-state 'next)
			  &allow-other-keys)
  (multiple-value-bind (NFA-instance begin-state)
      (push-state begin-state
		  NFA-instance)
    (multiple-value-bind (NFA-instance end-state)
	(push-state end-state
		    NFA-instance)
